...
protected:
	...
	virtual void dispatch()
	{
		...
		if (return_id_fn_)
			this->return_id_fn_(this->timerid);
	}

	std::function<void(unsigned long long)> return_id_fn_;

public:
	__WFTimerTask(time_t seconds, long nanoseconds, CommScheduler *scheduler,
		      timer_callback_t &&cb,
		      std::function<void(unsigned long long)> ret_id_fn = nullptr)
		: WFTimerTask(scheduler, std::move(cb)), return_id_fn_(ret_id_fn)
	{
...
WFTimerTask *WFTaskFactory::create_timer_task(time_t seconds, long nanoseconds,
					      ...
					      std::function<void(unsigned long long)> ret_id_fn)
{
	return new __WFTimerTask(seconds, nanoseconds, WFGlobal::get_scheduler(),
				 std::move(callback), std::move(ret_id_fn));
}
...
WFTimerTask *WFTaskFactory::create_timer_task(unsigned int microseconds, timer_callback_t callback)
{
	return WFTaskFactory::create_timer_task(
		microseconds / 1000000, microseconds % 1000000 * 1000, std::move(callback));
}
